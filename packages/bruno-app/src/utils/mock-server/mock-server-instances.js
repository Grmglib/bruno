...
export const getMockServerInstances = (state, workspaceUid) => {
  if (!workspaceUid) {
    return Object.values(state.mockServer?.instancesByWorkspace || {}).flat().filter(Boolean);
  }

  const instances = get(state.mockServer?.instancesByWorkspace, workspaceUid, []);
...
export const resolveInstanceSpec = (instance, apiSpecs = []) => {
  ...
  const specs = Array.isArray(apiSpecs) ? apiSpecs : [];
  const byPath = specs.find(
...
export const isMockServerNameTaken = (instances = [], name, excludeUid = null) => {
  ...
  if (!normalized || !Array.isArray(instances)) {
    return false;
  }

  return instances.some((instance) => (
    instance?.uid !== excludeUid
    && String(instance?.name || '').trim().toLowerCase() === normalized
  ));
};
