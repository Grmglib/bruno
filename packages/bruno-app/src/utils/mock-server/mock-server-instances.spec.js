...
describe('resolveInstanceSpec', () => {
  it('returns null when the instance has no spec path', () => {
    expect(resolveInstanceSpec({ sourceType: 'spec' }, [])).toBeNull();
  });

  it('tolerates a missing apiSpecs array', () => {
    const result = resolveInstanceSpec({
      sourceType: 'spec',
      specPath: 'C:/specs/petstore.yaml'
    }, null);

    expect(result).toEqual({
      pathname: 'C:/specs/petstore.yaml',
      name: 'petstore.yaml',
      filename: 'petstore.yaml'
    });
  });

  it('matches an open api spec by normalized path', () => {
    const apiSpecs = [{
      uid: 'spec-1',
      name: 'Petstore',
      pathname: 'C:\\specs\\petstore.yaml'
    ...
    expect(resolveInstanceSpec({
      sourceType: 'spec',
      specPath: 'C:/specs/petstore.yaml'
    }, apiSpecs)).toEqual(apiSpecs[0]);
  });
});
...
describe('mock server instance validation helpers', () => {
  ...
  it('handles instances with missing names', () => {
    expect(isMockServerNameTaken([{ uid: 'a', name: null }], 'Shop Mock')).toBe(false);
  });
