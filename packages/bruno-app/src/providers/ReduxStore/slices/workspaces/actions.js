...
import { createCollection, openCollection, openMultipleCollections, openScratchCollectionEvent, mountCollection, ensureActiveTabInCurrentWorkspace, removeCollection as removeCollectionAction, hydrateCollectionWithUiStateSnapshot } from '../collections/actions';
    ...
    try {
      ...
      if (!workspace.pathname) {
        ...
        if (Array.isArray(result)) {
          // Handles existing API that returns an array of collections directly
          collections = result.map((collection) => ({ ...collection }));
        } else {
          // Handles API that returns an object with collections and collectionGroups
